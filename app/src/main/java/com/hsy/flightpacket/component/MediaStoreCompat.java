...
import com.hsy.flightpacket.permission.PermissionTools;
    ...
    public String invokeCameraCapture(Activity activity, int requestCode) {
        if (!hasCameraFeature(this.mContext)) {
            ...
            if (toSave == null) {
                ...
                if (!PermissionTools.checkPermission(activity, PermissionTools.OP_CAMERA)) {
                    Toast.makeText(activity, "相机被禁用，请到系统设置开启权限！", Toast.LENGTH_LONG).show();
                    return null;
                }
